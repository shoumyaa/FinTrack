...
def generate_alerts(df, income, expense, savings_rate, user_id):
    alerts = []
    if df.empty:
    ...
    budgets = load_budgets(user_id, current_month)
    if not budgets.empty and not this_month.empty:
        ...
        for _, row in budgets.iterrows():
            ...
            if pct > 100:
                ...
                alerts.append(("warn","⚠️",f"<b>{cat}</b> at <b>{pct:.0f}%</b> with <b>{days_left} days</b> left."))
    if savings_rate < 10 and income > 0:
        alerts.append(("bad","📉",f"Savings rate is only <b>{savings_rate:.1f}%</b>. Aim for at least 20%."))
    elif savings_rate >= 30:
        alerts.append(("good","🎉",f"Great job! Saving <b>{savings_rate:.1f}%</b> of income."))
    if not df[df["type"]=="Expense"].empty:
        top = df[df["type"]=="Expense"].groupby("category")["amount"].sum().idxmax()
        top_amt = df[df["type"]=="Expense"].groupby("category")["amount"].sum().max()
        alerts.append(("info","📊",f"Biggest expense: <b>{top}</b> at <b>{inr(top_amt)}</b>."))
    if not alerts:
...
def render(user_id):
    ...
    budgets_month = load_budgets(user_id, date.today().strftime("%Y-%m"))
    adherence = 0.7
    ...
    with col_score:
        ...
        fig_gauge.update_layout(
            ...
            margin=dict(t=10,b=0,l=20,r=20), height=200,
    ...
    with col_kpis:
        ...
        <div class="kpi-grid">
          ...
          <div class="kpi-card ro"><div class="kpi-glow"></div>
            <span class="kpi-icon">💸</span>
        ...
        alerts = generate_alerts(df, income, expense, rate, user_id)
        alerts_html = ""
    ...
    if df.empty:
        st.info("No transactions yet — add some to see charts!"); return
